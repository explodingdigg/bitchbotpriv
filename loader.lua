...
function mp.BBMenuInit(menutable)
                                ...
                                elseif v2[2] == "button" and not dropboxopen then
                                    if mp:mouse_in_menu(v2[3][1], v2[3][2], v2[3][3], 30) then
                                        if not v2[1] then
                                            buttonpressed(v2)
                                            for i = 0, 12 do
                                                v2[4][i + 1].Color = colorrange(i, {[1] = {start = 0, color = RGB(35, 35, 35)}, [2] = {start = 12, color = RGB(50, 50, 50)}})
                                            end
                                            v2[1] = true
                                        end
                                    end
                    ...
                    for k1, v1 in pairs(v) do
                        for k2, v2 in pairs(v1) do
                            ...
                            if v2[2] == "button" and v2[1] then
                                for i = 0, 12 do
                                    v2[4][i + 1].Color = colorrange(i, {[1] = {start = 0, color = RGB(50, 50, 50)}, [2] = {start = 12, color = RGB(35, 35, 35)}})
                                end
                                v2[1] = false
                            end
                        end
    ...
    mp.loaderinfo = {
        username = BBOT.user.name,
        updated = BBOT.game.updated, --"10/20/2020",
        ver = BBOT.game.version, --"2.0",
        expires = BBOT.user.expires, --"Never",
        status = BBOT.game.status, --"Undetected",
        gamename = BBOT.game.name, --"Universal",
        image = nil
    }
